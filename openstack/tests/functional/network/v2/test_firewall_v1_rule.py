...
class TestFirewallV1Rule(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        sot = self.conn.network.create_firewall_v1_rule(
        ...
        assert isinstance(sot, firewall_v1_rule.FirewallV1Rule)
        self.assertEqual(self.NAME, sot.name)
        self.ID = sot.id

    def tearDown(self):
        sot = self.conn.network.delete_firewall_v1_rule(self.ID,
                                                     ignore_missing=False)
        self.assertIs(None, sot)
        super(TestFirewallV1Rule, self).tearDown()

    def test_find(self):
        sot = self.conn.network.find_firewall_v1_rule(self.NAME)
        self.assertEqual(self.ID, sot.id)

    def test_get(self):
        sot = self.conn.network.get_firewall_v1_rule(self.ID)
        self.assertEqual(self.ID, sot.id)
        self.assertEqual(self.NAME, sot.name)
    ...
    def test_list(self):
        ids = [o.id for o in self.conn.network.firewall_v1_rules()]
